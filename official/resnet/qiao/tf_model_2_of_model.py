...
def save2file(numpy_array, of_model_path, name):
  ...
  if name == 'weight' and ('fc1001' in numpy_array):
    tf_model = tf_model.transpose(1, 0)
  elif name == 'weight' and ('fc1001' not in numpy_array):
...
def convert(of_model_path, tf_model_path):
  weight_path = os.path.join(tf_model_path, 'kernel.npy')
  bias_path = os.path.join(tf_model_path, 'bias.npy')
  beta_path = os.path.join(tf_model_path, 'beta.npy')
  gamma_path = os.path.join(tf_model_path, 'gamma.npy')
  ...
  if os.path.exists(gamma_path):
    save2file(gamma_path, of_model_path, 'gamma')
